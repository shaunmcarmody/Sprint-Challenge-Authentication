...
1. What is the purpose of using _sessions_?
  ...
  With HTTP being a stateless protocol, by default no context is recieved from each request from the client. Sessions allow context to be provided between client and server, this context can be used to establish identity.

2. What does bcrypt do to help us store passwords in a secure manner.

  Bcrypt will replace plain text passwords with a cryptographic hash, which are much safer to store.

3. What does bcrypt do to slow down attackers?

  Bcrypt incorporates a salt in to the hash method.
